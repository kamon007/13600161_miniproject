...
class ViewController: UIViewController {
    ...
    @objc func processTime() {
        ...
        //showtimer.text = String(timeCount) Casting
            ...
            ProgressHUD.showError("Time out")
            timer.invalidate()
    ...
    func checkAnswer(){
        ...
        if correctAnswer == pickedAnswer{
            ...
            ProgressHUD.showSuccess("ดี")
            playSound("WOW0")
        }else{
            print("ผมเสียใจ")
            score = score + 1
            scoreFalse = scoreFalse + 1
            ProgressHUD.showError("ไอควาย")
            playSound("OHNO")
            nextQuestion()
    ...
    func nextQuestion() {
        ...
        if questionNumber <= 4 {
            ...
            let alert = UIAlertController(title: "WOW", message: "ควายคะมึงคือ \(scoreTure)", preferredStyle: .alert)
...
//
//        crrectAnsLabel.text = "Correct Ans: " + String(scoreTure)+" = " + String((Float)(scoreTure*100)/5)
//        wrongAnsLabel.text = "Wrong Ans: " + String(scoreFalse)+" = " + String((Float)(scoreFalse*100)/5)
//        nextQuestion()
//
//    }
